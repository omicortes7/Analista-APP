...
// ─── FIX: Añadir campos Fortalezas y Mejoras al formulario de informe ───
(function() {
  function patchInformeForm() {
    // Esperar a que renderDT esté disponible y parchear tab informe
    var _origRenderDT = window.renderDT;
    if(!_origRenderDT) return setTimeout(patchInformeForm, 400);

    window.renderDT = function(tab) {
      _origRenderDT(tab);
      if(tab !== 'informe') return;

      // Inyectar campos de fortalezas y mejoras después del formulario existente
      setTimeout(function() {
        var body = document.getElementById('djbody');
        if(!body) return;

        // Buscar si ya están inyectados
        if(document.getElementById('inf-positivos-wrap')) return;

        // Buscar el textarea de notas para insertar antes del botón guardar
        var notasTA = document.getElementById('inf-notas');
        if(!notasTA) return;

        var parentCard = notasTA.parentElement;
        if(!parentCard) return;

        // Crear los campos de fortalezas y mejoras
        var wrapper = document.createElement('div');
        wrapper.id = 'inf-positivos-wrap';
        wrapper.style.cssText = 'margin-top:.875rem;';

        var SI = 'width:100%;border:0.5px solid var(--border2);border-radius:var(--radius-sm);padding:8px;font-size:12px;background:var(--bg);color:var(--text);resize:vertical;font-family:inherit;outline:none;line-height:1.5;box-sizing:border-box;margin-bottom:8px;';

        wrapper.innerHTML =
          '<div style="display:grid;grid-template-columns:1fr 1fr;gap:8px;margin-bottom:8px;">' +
            '<div>' +
              '<div style="font-size:10px;font-weight:700;color:#3fb950;margin-bottom:4px;">✓ Fortalezas del jugador</div>' +
              '<textarea id="inf-positivos" rows="3" placeholder="Lo que hizo bien en este partido..." style="' + SI + 'border-color:rgba(63,185,80,0.3);"></textarea>' +
            '</div>' +
            '<div>' +
              '<div style="font-size:10px;font-weight:700;color:#f85149;margin-bottom:4px;">△ Aspectos a mejorar</div>' +
              '<textarea id="inf-mejoras" rows="3" placeholder="Lo que debe trabajar..." style="' + SI + 'border-color:rgba(248,81,73,0.3);"></textarea>' +
            '</div>' +
          '</div>';

        // Insertar antes del textarea de notas
        parentCard.insertBefore(wrapper, notasTA.previousElementSibling || notasTA);

        console.log('✅ Campos fortalezas/mejoras inyectados');
      }, 200);
    };
  }
  setTimeout(patchInformeForm, 500);

  // Parchear saveInforme para guardar fortalezas y mejoras
  function patchSaveInforme() {
    var _origSave = window.saveInforme;
    if(!_origSave) return setTimeout(patchSaveInforme, 600);

    window.saveInforme = async function() {
      var positivos = ((document.getElementById('inf-positivos')||{}).value||'').trim();
      var mejoras   = ((document.getElementById('inf-mejoras')  ||{}).value||'').trim();

      // Llamar al saveInforme original
      await _origSave();

      // Si hay campos nuevos, actualizar el último informe guardado
      if((positivos || mejoras) && window.state && window.state.informesPartido && window.state.informesPartido[0]) {
        var lastInf = window.state.informesPartido[0];
        var db = window.getDB ? window.getDB() : window.DB_REF;
        if(db && lastInf && lastInf.id) {
          db.from('informes_partido').update({
            positivos: positivos,
            mejoras: mejoras
          }).eq('id', lastInf.id).then(function(res) {
            if(!res.error) {
              lastInf.positivos = positivos;
              lastInf.mejoras   = mejoras;
            }
          });
        }
      }
    };
    console.log('✅ saveInforme patcheado con fortalezas/mejoras');
  }
  setTimeout(patchSaveInforme, 600);
})();
