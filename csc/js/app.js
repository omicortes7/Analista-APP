...
function renderInicio(){
  ...
  document.getElementById('hpend').innerHTML=pend.length?pend.map(j=>{const ss=ssBadge(j.sesion_fecha);return`<div style="display:flex;align-items:center;gap:8px;padding:7px 0;border-bottom:0.5px solid var(--border);font-size:12px;"><div style="width:7px;height:7px;border-radius:50%;background:${ss.col};flex-shrink:0;"></div><div style="flex:1;"><b>${j.nombre}</b> · ${j.posicion}</div><span class="badge" style="background:${ss.col}20;color:${ss.col};">${ss.l}</span></div>`;}).join(''):'<div style="font-size:12px;color:var(--text3);padding:6px 0;">No hay sesiones programadas.</div>';
}
...
function renderJugadores(){
  ...
  document.getElementById('jptabs').innerHTML=['Todas',...POSICIONES].map(p=>`<button class="btn-outline btn-sm${state.filtros.jugadores.pos===p?' active':''}" style="${state.filtros.jugadores.pos===p?'background:var(--text);color:var(--bg);':''}" onclick="setJP('${p}')">${p}</button>`).join('');
  ...
  g.innerHTML=f.map(j=>{
    ...
    const lo=getObsJugador(j.id)[0];
    return`<div class="card" onclick="openJug('${j.id}')">
      <div style="display:flex;align-items:center;gap:8px;margin-bottom:8px;">
        ${j.logo_club?`<img src="${j.logo_club}" style="width:36px;height:36px;object-fit:contain;border-radius:4px;" onerror="this.style.display='none'">`:''}
        <div class="avatar" style="width:36px;height:36px;font-size:12px;background:${pc.bg};color:${pc.color};">${initials(j.nombre)}</div>
        <div><div style="font-size:13px;font-weight:500;">${j.nombre}</div><div style="font-size:11px;color:var(--text2);">${j.posicion} · ${j.equipo||''} · ${j.categoria||''}</div></div>
      </div>
      <div style="border-top:0.5px solid var(--border);padding-top:8px;">
        <div style="display:flex;justify-content:space-between;font-size:11px;margin-bottom:3px;"><span style="color:var(--text3);">Objetivos</span><span>${getObjJugador(j.id).length}</span></div>
        <div style="display:flex;justify-content:space-between;font-size:11px;margin-bottom:8px;"><span style="color:var(--text3);">Último partido</span><span>${lo?lo.partido:'—'}</span></div>
        <span class="badge" style="background:${ss.col}20;color:${ss.col};font-size:11px;">${ss.l}</span>
        <button onclick="event.stopPropagation();copiarURLJugador('${j.id}')" style="margin-top:8px;background:none;border:0.5px solid rgba(124,111,240,.4);border-radius:99px;padding:3px 10px;font-size:10px;cursor:pointer;color:#7C6FF0;width:100%;">🔗 Copiar enlace del jugador</button>
      </div></div>`;
  }).join('');
}
